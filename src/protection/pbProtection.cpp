...
bool pbProtection::checkProtectionCode( const string &fileName, string &reason )
{
	...
	if ( list.size() >= 2 ) {
		...
		if ( !result ) reason = "Bad licence number or image files are changed/corrupted";
		else {	
			if ( param.date ) {
				//проверка даты
				int year = ofGetYear();
...
void pbProtectionSetup( ofxArgs *args )		//считывание и проверка лицензии
{
	...
	if ( protectionOk )	cout << "Licence: ok" << endl;
	else cout << endl << "ERROR: Bad licence - " << reason << endl;
	if ( !protectionOk ) {
