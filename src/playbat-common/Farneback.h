...
public:
	...
	Mat *flow() { return &_Flow; }
	bool empty() { return ( width() == 0 || height() == 0 ); }

	unsigned char *outImageRGBA() { if ( _outImage.size() > 0 ) return &_outImage[0]; else return 0;}
	
	int width()		{	return _Flow.cols;	}
	int height()	{	return _Flow.rows;	}
