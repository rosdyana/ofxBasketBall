...
void pbBaseApp::draw0()
{
	...
	if ( _drawMode == 0 ) drawProduction();
	if ( _drawMode == 1 ) drawDebug();
...
void pbBaseApp::keyPressed0(int key)
{
	switch (key){
		case 'z':					
			...
			return;
			break;
		case 't': runInfo.setPrintFps( !runInfo.printFps() );	
			return;
			break;

		case 'd': _drawMode = 1 - _drawMode;				
			return;
			break;
		case 'f': _debugModeAdditional = 1 - _debugModeAdditional;	
			return;
			break;
		case 'g': _debugModeDrawCamera = 1 - _debugModeDrawCamera;
			return;
			break;
