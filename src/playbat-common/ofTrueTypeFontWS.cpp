...
//Denis ввел эту константу, иначе для пробелов получался -1, и вылетало с ошибкой при расчете ширины строки.
#define NUM_CHARACTER_TO_START_WS (0)
...
static const int NUM_OF_ALL_CHARCTERS = 100000; // (int)L'！';
...
void ofTrueTypeFontWS::loadFont(string filename, int fontsize, bool _bAntiAliased, bool _bFullCharacterSet, bool makeContours){
  ...
  nCharacters = bFullCharacterSet ? NUM_OF_ALL_CHARCTERS : 128 - NUM_CHARACTER_TO_START_WS;
  ...
  //---------------- 
  int cy = (int)L'p' - NUM_CHARACTER_TO_START_WS;
...
ofRectangle ofTrueTypeFontWS::getStringBoundingBox(wstring c, float x, float y){
  ...
  bool bFirstCharacter = true;
	while(index < len){
		int cy = (int)c[index] - NUM_CHARACTER_TO_START_WS;
...
static wstring WSFont_strToWstr( const string &text )
{
...
//=====================================================================
void ofTrueTypeFontWS::drawString(string s, float x, float y)
{
	drawString( WSFont_strToWstr( s ), x, y );
}
...
void ofTrueTypeFontWS::loadEachChar(int char_id, int fontsize, bool _bAntiAliased, bool _bFullCharacterSet) {
  int i = char_id;
  //------------------------------------------ anti aliased or not:
  if(FT_Load_Glyph( face, FT_Get_Char_Index( face, (int)(i+NUM_CHARACTER_TO_START_WS) ), FT_LOAD_DEFAULT )){
  ...
  if(bMakeContours){
    if( printVectorInfo )printf("\n\ncharacter %c: \n", wchar_t( i+NUM_CHARACTER_TO_START_WS ) );
    
    //int character = i + NUM_CHARACTER_TO_START_WS;
