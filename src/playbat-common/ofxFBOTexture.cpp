...
//-----------------------------------------------------------------------
void ofxFBOTexture::getImage(ofImage &image)
{
    begin();
    int x = 0;
    int y = 0;
    ...
    glReadPixels(x, y, w, h, GL_RGB, GL_UNSIGNED_BYTE, pixels);		// read the memory....
#ifndef TARGET_OF_IPHONE
    glPopClientAttrib();
#endif

    /*int sizeOfOneLineOfPixels = w * 3;
    unsigned char * tempLineOfPix = new unsigned char[sizeOfOneLineOfPixels];
    unsigned char * linea;
    unsigned char * lineb;
    for (int i = 0; i < h/2; i++){
    	linea = pixels + i * sizeOfOneLineOfPixels;
    	lineb = pixels + (h-i-1) * sizeOfOneLineOfPixels;
