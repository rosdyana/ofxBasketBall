...
ofxVec2f ofToVec2f(string str)
{
    ...
    if (v.size() == 1) {
        return ofxVec2f(ofToFloat(v[0]), ofToFloat(v[0]));    ///is dit gewenst?
    }
    if (v.size() != 2) {
        return ofxVec2f(0, 0);
    } else {
        return ofxVec2f(ofToFloat(v[0]), ofToFloat(v[1]));
    }
}
...
ofxVec3f ofToVec3f(string str)
{
    vector <string> v = ofSplitString(str);
    if (v.size() == 1) {
        return ofxVec3f(ofToFloat(v[0]), ofToFloat(v[0]), ofToFloat(v[0]));    ///is dit gewenst? .5 wordt dus (.5,.5,.5) wordt oa gebruikt voor scale.
    }
    if (v.size() != 3) {
        return ofxVec3f(0, 0, 0);
    } else {
        return ofxVec3f(ofToFloat(v[0]), ofToFloat(v[1]), ofToFloat(v[2]));
    }
}
...
ofxVec4f ofToVec4f(string str)
{
    vector <string> v = ofSplitString(str);
    if (v.size() != 4) {
        return ofxVec4f(0, 0, 0, 0);
    } else {
        return ofxVec4f(ofToFloat(v[0]), ofToFloat(v[1]), ofToFloat(v[2]), ofToFloat(v[3]));
    }
}
...
bool ofxIniSettings::load(string filename, bool clearFirst)
{
    ...
    if (filename == "") {
        die("ofxIniSettings::no filename");
    }
...
string   ofxIniSettings::get(string key, string   defaultValue)
{
    return has(key) ? keys[key] : defaultValue;
}
int      ofxIniSettings::get(string key, int      defaultValue)
{
    return has(key) ? ofToInteger(keys[key]) : defaultValue;
}
float    ofxIniSettings::get(string key, float    defaultValue)
{
    return has(key) ? ofToFloat(keys[key]) : defaultValue;
}
bool     ofxIniSettings::get(string key, bool     defaultValue)
{
    return has(key) ? ofToBoolean(keys[key]) : defaultValue;
}
ofxVec2f ofxIniSettings::get(string key, ofxVec2f defaultValue)
{
    return has(key) ? ofToVec2f(keys[key]) : defaultValue;
}
ofxVec3f ofxIniSettings::get(string key, ofxVec3f defaultValue)
{
    return has(key) ? ofToVec3f(keys[key]) : defaultValue;
}
ofxVec4f ofxIniSettings::get(string key, ofxVec4f defaultValue)
{
    return has(key) ? ofToVec4f(keys[key]) : defaultValue;
}
ofRectangle ofxIniSettings::get(string key, ofRectangle defaultValue)
{
    return has(key) ? ofToRectangle(keys[key]) : defaultValue;
}

void ofxIniSettings::setString(string newID, string newValue)
{
    if (outputFilename == "") {
        die("ofxIniSettings::setString: outputFilename undefined");
    }
    bool foundKey = false;
    //outputFilename = ofToDataPath(outputFilename);
    if (!ofFileExists(outputFilename)) {
        die("ofxIniSettings::setString: file not found: " + outputFilename);
    }
    string cmd, section, key, value, id;
