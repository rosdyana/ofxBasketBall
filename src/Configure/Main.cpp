	...
	: TForm(Owner)
{
	...
	for (int i=0; i<duration.size(); i++) {
		...
		if ( minute > 0 ) {
			...
			if ( minute == 1 ) title += "Minute"; else title += "Minutes";
		}
		if ( sec > 0)
		{
			if ( title != "" ) title += " ";
...
void TMainForm::readIni()
{
	...
	{
		int index = 0;
		if ( fullScreen == "1" ) index = 1;
		if ( fullScreen == "2" ) index = 2;
	...
	//Камера
	{
		...
		if ( cameraUsePS3 == "1" ) CameraTypePS3Radio->Checked = true;
		if ( cameraUseKinect == "1" ) CameraTypeKinectRadio->Checked = 2;
	}
