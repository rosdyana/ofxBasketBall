...
public:
    ...
    float value() const
    {
        return _value.value();    //Текущее значение, 0..1
    }
    bool isOn() const
    {
        return value() > 0;
    }
    float lastEnableTime() const
    {
        return _lastEnableTime;    //Время последнего включения (после полного выключения)
    }

    cv::Rect_<float> rect() const
    {
        return _rect;
    }
