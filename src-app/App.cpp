...
int App::w() { 	return _w; }
int App::h() { 	return _h; }
int App::frameRate() { return _frameRate; }
...
void App::setup()
{
...
#ifdef SHOW_LOGO
	{
		CircleData c;
		c.rad		= 40;
		c.p			= ofPoint( ofRandom( c.rad, w - c.rad ), ofRandom( c.rad, h / 2 ) );
		c.angleDeg		= 0.0;
		c.groupIndex	= 1;		//тип круга - для рендеринга
		c.density		= 10.0;//1.0;
		c.friction		= 1.0;		//трение
		c.restitution	= 0.4;		//отскок
